...
const triggerConfetti = ({ mode = "mini", sourceX, sourceY } = {}) => {
...
const playVideoInsidePhoto = (photo, videoSrc) => {
  if (!videoSrc || photo.classList.contains("playing")) return;

  const previous = photo.querySelector(".photo-overlay-video");
  if (previous) {
    previous.remove();
  }
...
const onPhotoPress = (photo, index) => {
